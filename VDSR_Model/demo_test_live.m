...
main_path = '/data/Yao/Data_Math_Project/Demo_Test_data_live/Color_Correction_live/Test_img_color_corred_result_finalData/kidney01_image_color_corrected.tif';
save_path = './Test_result_YCbCr_live_finalData/';
...
%% test
%folders = dir(fullfile(main_path,'*_corred'));
...
%for fld_i = 1:length(folders)
   ...
   % save_path = strcat(save_main_path);
    if ~exist(save_path,'dir'), mkdir(save_path); end
    
    %for i = 1:length(files_LR)
        img_LR = imread(main_path);
...
%         figure, imshow(img_LR); title('LR Y channel 0-255');
        ...
        imwrite(img_sr_rgb,fullfile(save_path,'kidney01_image_color_VDSR_sr.tif'));
 %   end
