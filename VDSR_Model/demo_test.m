...
main_path = '../../../Demo_Test_data/';
save_main_path = './Test_result_YCbCr/';
...
%% test
folders = dir(fullfile(main_path,'*_pair'));
...
for fld_i = 1:length(folders)
    ...
    save_path = strcat(save_main_path,folders(fld_i).name);
...
%         figure, imshow(img_LR); title('LR Y channel 0-255');
        ...
        imwrite(img_sr_rgb,fullfile(save_path,strrep(files_GT(i).name,'GT','SR')));
    end
