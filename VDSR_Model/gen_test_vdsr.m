...
for folder_i = 1:length(folders_CC)
    ...
    savefile = fullfile(savepath,strrep(folders_CC(folder_i).name,'_Corrected','_test.h5'));
    ...
    load(fullfile(savepath,'split.mat'));
    
    files_cc_test = files_cc(~tf);
    files_gt_test = files_gt(~tf);
    ...
    for i = 1 : length(files_cc_test)
        
        img_cc = imread(fullfile(path_CC,folders_CC(folder_i).name,char(files_cc_test(i))));
        img_gt = imread(fullfile(path_GT,folders_GT(folder_i).name,char(files_gt_test(i))));
